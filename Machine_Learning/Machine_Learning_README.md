...
- **OGs exclusive of a non-only aquatic phylum**: I’m not sure if for phyla that have aquatic and terrestrial species, these OGs are informative or not in the model (if it will get me only common to all events or also about some events inside phyla). I’m thinking for example about the arthropods that have several terrestrialization events, and maybe the model can help us identify OGs that originated in arthropods but that were lost/duplicated/whatever later on independently. Also, and maybe more importantly, I’m working on gene counts not presence/absence. So, it could be that the gene is present in all species in the phylum but has more copies in the terrestrial ones.
- **OGs with low taxon occupancy**: For other analyses in the lab, we have filtered OGs that are present in less than 20% of the species that should be present on a given node. This allows the removal of putative contaminants. In [previous tests](https://www.notion.so/Obtaining-OGs-and-other-files-c7189bfeca5245e5b2ed9e4a72d344f0?pvs=21), using this filter removed OGs that were found to be important in the model (with the first 10k subset). I’m afraid that doing this filter might remove interesting genes that are very old and are only found in some lineages (the proportion of species descending from that node that have the gene will be low), as the example found before.
